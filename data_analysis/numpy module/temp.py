import numpy as np
import os
#to save data
"""data = np.array([
    ['03-10-16', 774.25, 776.065002, 769.5, 772.559998],
    ['04-10-16', 776.030029, 778.710022, 772.890015, 776.429993],
    ['05-10-16', 779.309998, 782.070007, 775.650024, 776.469971],
    ['06-10-16', 779, 780.47998, 775.539978, 776.859985],
    ['07-10-16', 779.659973, 779.659973, 770.75, 775.080017]
], dtype=object)
np.savetxt("test.csv",data,delimiter=',',fmt='%s',header='Date,Open,High,Low,Close',comments='')"""

#to load data
"""csvdata=np.genfromtxt('test.csv', dtype=['S10', 'float32', 'float32', 'float32', 'float32'], delimiter=",")
print(csvdata)"""